#!/usr/bin/env bash
# Shared Darwin/Apple-mobile compat headers + source tweaks for foot.
# Env: LINUX_INPUT_HEADERS = path to linux/input-event-codes.h
set -euo pipefail
: "${LINUX_INPUT_HEADERS:?}"
    ...
    cp "$LINUX_INPUT_HEADERS" compat/linux/input-event-codes.h
